...
func splitParse(toSplit tokenType, subNodeType nodeType, tokens []token, root tree, tokenValues ...string) tree {
	for i, currentToken := range tokens {
		if currentToken.group == toSplit && acceptTokenValue(currentToken.value, tokenValues) {
			...
			//build left subtree
			subNode = parser(tokens[0:i], subNode)
			//build right subtree
			subNode = parser(tokens[i+1:len(tokens)], subNode)
...
//astBuilder takes a slice of tokens, generated by tokenizer,
//	and splits the code into statements,
//	which are then parsed by parser
func astBuilder(tokens []token, root tree) tree {
...
//parser parses an array of tokens, given to it by astBuilder, into an AST.
...
func parser(tokens []token, root tree) tree {
	...
	//	as currently parser does not have exclusive access to root
